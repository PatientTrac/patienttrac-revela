        ...
        If Not IsNull(rsFac.Fields("FACILITY_NAME")) Then
            ...
            If strCompareFac <> vbNullString Then
                ...
                Else
                    strCompareFac = Trim(strCompareFac)
                End If
                strCompareFac = IsProperCase(Trim(strCompareFac))
...
Dim lngPrimKey As Long
    ...
    strSQL = "SELECT START_DATE, END_DATE,  PROVIDER_ID, BLOCK_ID, " _
        & "START_TIME, END_TIME, REASON, DURATION, FACILITY_ID, DAYWEEK FROM APPOINTMENTS_BLOCKS  WHERE " _
        & "PROVIDER_ID = " & vlngProvider & " AND ACTIVE = 1 ORDER BY CONVERT(DATETIME, START_DATE, 101) DESC, CONVERT(DATETIME, START_TIME, 108)  "
...
Dim lngPrimKey As Long
    ...
    If cnBlock.State = adStateClosed Then cnBlock.open psConnect(1)
    strSQL = "SELECT CONVERT(DATETIME, START_DATE, 101) AS START_DATE, CONVERT(DATETIME, END_DATE, 101) AS END_DATE, PROVIDER_ID, BLOCK_ID, " _
        & "START_TIME, END_TIME, REASON, DURATION, FACILITY_ID, DAYWEEK FROM APPOINTMENTS_BLOCKS  WHERE " _
        & "PROVIDER_ID = " & udtBlock.ProviderID & " AND ACTIVE = 1 AND " _
        & "convert(DATETIME, START_DATE, 101)  BETWEEN '" & Format(udtTime.StartDate, "mm/dd/yyyy") & "' AND " _
        & "'" & Format(udtTime.EndDate, "mm/dd/yyyy") & "' ORDER BY CONVERT(DATETIME, START_DATE, 101) DESC, CONVERT(DATETIME, START_TIME, 108)  "
        Set rsBlock = cnBlock.Execute(strSQL)
    Do Until rsBlock.EOF
        Clear_BlockTerm
        Set Flds = rsBlock.Fields
   ...
   If Trim(udtBlock.StartDate) = vbNullString Then
    ...
    If cnTerms.State = adStateClosed Then cnTerms.open psConnect(1)
     strSQL = "SELECT BLOCK_ID, START_TIME, END_TIME, START_DATE, END_DATE FROM APPOINTMENTS_BLOCKS WHERE PROVIDER_ID = " & udtBlock.ProviderID & " AND " _
        ...
        & "CONVERT(VARCHAR, START_DATE, 10) < '" & Format(strEndDate, "mm/dd/yyyy") & "'  ORDER BY START_TIME DESC"
        Set rsTerms = cnTerms.Execute(strSQL)
        With rsTerms
            Clear_Block
            Do Until .EOF
...
Dim lngApptID As Long
    ...
    Else
        strSQL = "SELECT APPOINTMENTS.*, PATIENT.LAST_NAME  FROM APPOINTMENTS INNER JOIN PATIENT ON APPOINTMENTS.PATIENT_ID = PATIENT.PATIENT_ID " _
            & "WHERE CONVERT(VARCHAR, APPOINTMENTS.APPT_DATE, 10) = '" & udtBlock.StartDate & "' AND " _
            & "APPOINTMENTS.PROVIDER_ID = " & udtBlock.ProviderID & " AND APPOINTMENTS.FACILITY_ID = " & udtBlock.FacilityID & " AND " _

...
Dim Arr
    ...
    If Arr(1) = 15 Then clsApptPref.SchedInterval = "EVERY 15 MIN" Else clsApptPref.SchedInterval = "EVERY 20 MIN"
    For Idx = mStartIdx To UBound(mStartHour)
        clsGridFormat.List = mStartHour(Idx) & " AM"
        If UCase(clsApptPref.SchedInterval) = "EVERY HALF HOUR" Then
...
'    strSQL = "Read_BlockTime_ProviderID " & vlngProvider
    strSQL = "SELECT CONVERT(DATETIME, START_DATE, 101) AS START_DATE, " _
        ...
        & "CONVERT(VARCHAR(10), START_DATE, 101) >  '" & Format(strDate, "mm/dd/yyyy") & "' ORDER " _
...
Private Sub cmdSelect_Click()
    ...
    End If
    If IsDate(udtTime.StartDate) And IsDate(udtTime.EndDate) Then
        ...
        End If
    Else
        MsgBox "Please select a start date and end date!", vbInformation, "PatientTrac Block Appointments"
        Exit Sub
    End If
    Exit Sub
End Sub

...
Begin VB.Form frmPsychiatry 
   AutoRedraw      =   -1  'True
   BackColor       =   &H00E0E0E0&
   Caption         =   "PSYCHIATRIC CLINICAL DATA"
   ClientHeight    =   11190
   ClientLeft      =   3855
   ClientTop       =   3285
   ClientWidth     =   14175
   ...
   End
End
Attribute VB_Name = "frmPsychiatry"
...
Private MyForm As FRMSIZE
Private DesignX As Integer
Private DesignY As Integer
...
Dim Arr
    ...
    On Error GoTo Error_Load_Psychiatry
  
    piPatientID = frmPatients.piPatientID
    piEncounterID = frmEnctr.piEncounterID
    Get_Encounter frmEnctr.piEncounterID
    ...
    ElseIf piEncounterID = 0 Then
        MsgBox "Please select Encounter First!", vbInformation
        Exit Sub
    End If
    piSelEncntrID = piEncounterID

    If mbTherapy = False Then
            strOCX = "FN_PsyMse"
            strControl = "NursProg"
        Else
            strOCX = "FN_PsyMse"
    ...
    Set fobjOCX = Controls.Add(strOCX & "." & strControl, "ctl" & strControl, Me)
        ...
        frmPsychiatry.Width = fobjOCX.Width + 300
...
Error_Load_Psychiatry:
    ...
    ErrHandler _
     "frmPsychiatry.Load_Psychiatry", _
     Err.Number, _
     Err.Description
     Screen.MousePointer = vbDefault
     Exit Sub
...
Private Sub Current_Enctr_Click()
    ...
    frmEnctr.piEncounterID = piEncounterID
    fobjLib.EncounterID = piEncounterID
    ...
    Else
        mbooNext = False
    End If
    If mlngPrevEnctrID = 0 Then Get_PrevEncounter piEncounterID
...
'    MyForm.Width = Me.Width
    ...
    LogFile "Start Log  Psychiatry --------------------", True

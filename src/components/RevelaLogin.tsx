...
export default function RevelaLogin({ onAuthenticated }: Props) {
  ...
  const handleCredentials = async () => {
    ...
    try {
      ...
      // Only providers can access Revela directly
  ...
  const S = {
    ...
    digit:  (f = false) => ({ width: '100%', textAlign: 'center' as const, fontSize: 22, fontWeight: 700, padding: '12px 0', background: f ? 'rgba(201,169,110,0.15)' : 'rgba(255,255,255,0.05)', border: `1px solid ${f ? 'rgba(201,169,110,0.5)' : 'rgba(255,255,255,0.1)'}`, borderRadius: 8, color: '#c9a96e', fontFamily: 'monospace' } as React.CSSProperties),
  }
      ...
      <div style={S.card}>
        <div style={S.hdr}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
            ...
            <span style={{ color: '#c9a96e', fontSize: 20, fontWeight: 600 }}>Revela</span>
            <span style={{ color: 'rgba(255,255,255,0.2)', fontSize: 13 }}>by PatientTrac</span>
          </div>
          <div style={{ color: 'rgba(255,255,255,0.35)', fontSize: 11, letterSpacing: '1.5px', textTransform: 'uppercase' }}>
            {step === 'credentials' ? 'Plastic Surgery EMR' : step === 'mfa_challenge' ? 'Two-Factor Authentication' : 'Set Up Google Authenticator'}
          </div>
        </div>
        ...
        <div style={S.body}>
          ...
          {step === 'credentials' && (<>
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', marginBottom: 24, lineHeight: 1.6 }}>
              Clinical documentation · Surgical notes · HIPAA Compliant
            </p>
            <label style={S.lbl}>Email Address</label>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && handleCredentials()}
              placeholder="surgeon@practice.com" style={S.inp}/>
          ...
          {step === 'mfa_challenge' && (<>
            <div style={{ background: 'rgba(0,212,255,0.08)', border: '1px solid rgba(0,212,255,0.2)', borderRadius: 10, padding: '14px 16px', marginBottom: 24 }}>
              ...
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', lineHeight: 1.6 }}>Open Google Authenticator on your phone and enter the code for PatientTrac Revela.</div>
            </div>
            ...
            <input autoFocus value={totpCode} onChange={e => handleTotpInput(e.target.value, setTotpCode)}
          ...
          {step === 'mfa_setup' && (<>
            ...
            <input autoFocus value={setupCode} onChange={e => handleTotpInput(e.target.value, setSetupCode)}

...
export default function RevelaLogin({ onAuthenticated }: Props) {
  ...
  const handleCredentials = async () => {
    ...
    try {
      ...
      if (!['provider', 'admin', 'super_admin'].includes(member.role)) throw new Error('Provider account required to access Revela.')
        ...
        <div style={S.body}>
          ...
          {step==='mfa_challenge' && (<>
            <div style={{ background:'rgba(0,212,255,0.08)', border:'1px solid rgba(0,212,255,0.2)', borderRadius:10, padding:'14px 16px', marginBottom:24 }}>
              ...
              <div style={{ fontSize:12, color:'rgba(255,255,255,0.5)', lineHeight:1.6 }}>Open Google Authenticator and enter the code for PatientTrac Revela.</div>
            </div>
